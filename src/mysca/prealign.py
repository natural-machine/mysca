...
def _align_mafft(
    ...
    *,
    ...
    n_in = _count_fasta(in_fasta)
    logger.info(
        "Aligning %d sequences with MAFFT --auto (threads=%d)", n_in, threads,
    )
    ...
    logger.info("Running: %s > %s", " ".join(argv), out_fasta)
    os.makedirs(os.path.dirname(os.path.abspath(out_fasta)), exist_ok=True)
    t0 = time.perf_counter()
    ...
    n_out = _count_fasta(out_fasta)
    logger.info(
        "Alignment done: %d sequences written in %.2fs", n_out, elapsed,
    )
    return {"n_in": n_in, "n_out": n_out, "elapsed_s": elapsed}
...
def run_align(
    ...
    *,
    ...
    """Align FASTA sequences, writing aligned FASTA to `out_fasta`."""
